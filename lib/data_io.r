...
### Write secondary aluminum data ###

write_2nd_alum <- function(datadir){

    # input static aluminum 
    second_aluminum_ggi <- data.frame(
        Source = c(rep("Primary", 5), rep("Scrap", 5), rep("Primary", 5), rep("Scrap", 5)), 
    ...
    second_aluminum_ggi_cus <- data.frame(
        Source = c(rep("Primary", 5), rep("Scrap", 5)),
